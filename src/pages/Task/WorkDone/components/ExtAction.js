 ...
 * @Last Modified time: 2021-12-09 18:16:17
 */
...
const menuData = () => [
  ...
  {
    title: formatMessage({ id: 'flowtask_000050', defaultMessage: '我要撤回' }),
...
class ExtAction extends PureComponent {
  ...
  getMenu = (menus, record) => {
    ...
    const flowInstanceId = get(record, 'flowInstance.id', null);
    return (
      <Menu
        id={menuId}
        ...
        {menus.map(m => {
          ...
          if (m.key === TASK_WORK_ACTION.FLOW_HISTORY) {
            return (
              <Item key={m.key}>
                <FlowHistoryButton
                  businessId={flowInstanceBusinessId}
